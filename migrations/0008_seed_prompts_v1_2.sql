-- Seed v1.2 of the clip_substance_scorer prompt.
-- Adds the `recommended_crop.horizontal_focus` output field so the vision
-- model can steer the 9:16 reframe horizontally instead of always cropping
-- the source center. Rule 7 ("Visual Clarity of Source") is extended with a
-- note to observe horizontal subject position, and a new "Crop focus rules"
-- section explains the [0.0, 1.0] convention (0.5 = current centered crop,
-- backward-compatible default).
--
-- Only `clip_substance_scorer` is bumped (v2 -> v3). The other three v1.1
-- prompts (hook_overlay_generator, hook_overlay_scorer, per_platform_post_text)
-- are unchanged and keep their existing version-2 rows.

INSERT OR IGNORE INTO prompts (key, version, body) VALUES
('clip_substance_scorer', 3,
